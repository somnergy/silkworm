...
Block StateTransition::get_block(InMemoryState& state, ChainConfig& chain_config) {
    ...
    block.header.gas_limit = std::stoull(get_env("gas"), nullptr, /*base=*/16);
    ...
    if (rev >= EVMC_CANCUN) {
        ...
        if (contains_env("excessBlobGas")) {
            block.header.excess_blob_gas = std::stoull(get_env("excessBlobGas"), nullptr, 0);
        }
        if (contains_env("blobGasUsed")) {
            block.header.blob_gas_used = std::stoull(get_env("blobGasUsed"), nullptr, 0);
        }
    }
...
Transaction StateTransition::get_txn_from_sub_state(const ExpectedSubState& expected_state) {
    ...
    // txn.nonce = j_transaction.at("nonce").get<std::uint64_t>();
    auto senderPtr = sender_to_address(j_transaction["from"]);
    ...
    txn.data = from_hex(j_transaction.at("input").get<std::string>()).value();

    txn.gas_limit = std::stoull(j_transaction.at("gas").get<std::string>(), nullptr, 16);
